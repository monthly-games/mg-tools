...
def firebase_status(game, all_games):
    ...
    for gid in game_ids:
        ...
        click.echo(
            f"MG-{gid:04d}  "
            f"{'✓' if status['firebase_options'] else '✗':<10}"
            f"{'✓' if status['google_services_json'] else '✗':<10}"
            f"{'✓' if status['google_service_info_plist'] else '✗':<10}"
            f"{'✓' if status['deps_enabled'] else '✗':<10}"
        )
...
def ads_status(game, all_games):
    ...
    for gid in game_ids:
        ...
        click.echo(
            f"MG-{gid:04d}  "
            f"{'✓' if status['has_config'] else '✗':<12}"
            f"{android_id:<30}"
        )
...
def config_show():
    ...
    click.echo(f"\nFirebase:")
    click.echo(f"  Project prefix: {config.firebase.project_prefix}")
    click.echo(f"  Dev project:    {config.firebase.dev_project_id}")
    click.echo(f"\nAds:")
